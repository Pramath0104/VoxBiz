...
const Signin = () => {
  ...
  const handleGoogleSignIn = async () => {
    toast.error("Google Login is currently under development.", { 
      duration: 3000,
      icon: '🚧'
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formParams.append("username", email); // FastAPI OAuth2 expects 'username'
      ...
      toast.error(error.response?.data?.detail || "Login failed", { duration: 3000 });
    }
  };
      ...
      <div className="relative z-20 flex w-full max-w-md flex-col items-center px-4">
        ...
        <div className="mb-8 text-center cursor-pointer" onClick={() => navigate("/")}>
           <img src="/voxlogo3_transparent.png" alt="VoxBiz" className="h-40 md:h-48 w-auto mx-auto object-contain drop-shadow-[0_4px_8px_rgba(0,0,0,0.5)]" />
            <p className="text-slate-500 dark:text-neutral-400 mt-2 text-sm">Sign in to your account</p>
        </div>
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <div className="flex justify-end mt-1">
              <a
                ...
                className="text-xs text-sky-500 hover:text-sky-600 dark:text-sky-400 dark:hover:text-sky-300 transition-colors"
              >
                Forgot Password?
              </a>
            </div>
